...
python src/s6_test.py \
    ...
    --data_type video \
    --data_path data_test/AI-TEST.mp4 \
...
if True:  # Include project path
    ...
    import utils.lib_face_recog as lib_face_recog # Import LBPHFaceRecognizer
...
def draw_result_img(img_disp, ith_img, humans, dict_id2skeleton,
                    skeleton_detector, multiperson_classifier,face_name,face_acc):
    ...
    if len(dict_id2skeleton):
        for id, label in dict_id2label.items():
            ...
            if len(dict_id2skeleton):
                ...
                action,action_acc = classifier_of_a_person.get_scores_image()


            # position = lib_plot.draw_action_result(img_disp, id, skeleton, face_name,face_acc, label,action_acc)
            position = lib_plot.draw_action_result(img_disp, id, skeleton, face_name,face_acc, action , action_acc)
    ...
    cv2.putText(img_disp, "Human-Pose-Detection",
                (10, 40), fontScale=0.75, fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                color=(0, 0, 0), thickness=3)

    cv2.putText(img_disp, "Frame: " + str(ith_img),
                (20, (40+40)), fontScale=1.5, fontFace=cv2.FONT_HERSHEY_PLAIN,
                color=(0, 0, 0), thickness=2)
    
    now = datetime.now()
    dt_string = now.strftime("%d/%m/%y %H:%M:%S")

    cv2.putText(img_disp, str(dt_string),
                (20, (40+80)), fontScale=1.5, fontFace=cv2.FONT_HERSHEY_PLAIN,
                color=(0, 0, 0), thickness=2)
    
    cv2.putText(img_disp, "github.com/newler55",
                (10, (680)), fontScale=0.75, fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                color=(0, 0, 0), thickness=2)

    # Draw predicting score for only 1 person
    if len(dict_id2skeleton):
        classifier_of_a_person = multiperson_classifier.get_classifier(id='min')
        classifier_of_a_person.draw_scores_onto_image(img_disp,face_name,face_acc,position)
    return img_disp
...
if __name__ == "__main__":
    ...
    try:
        ...
        while images_loader.has_image():
            ...
            face_person = lib_face_recog.FaceRecognition()
            face_name,face_acc = face_person.face_ai(img)
            ...
            if len(dict_id2skeleton):
                ...
                # print("prediced dict_id2skeleton :",dict_id2skeleton)
                # print("person = " + human_name + " , action = " + dict_id2label[min_id])
